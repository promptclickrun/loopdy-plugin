...
`socket-ready-v1,backpressure-v1` prefix to the signed
`x-loopdy-capabilities` upgrade header. The host appends
`cards-templates-v1` when the template workspace surface is available and
`marketplace-skills-hub-v1` only when marketplace release trust is configured.
After authentication, the Link service sends a `socket.ready` control
...
`capabilities` with `protocolVersion: 1`, `pluginVersion` from the loaded plugin,
features `["workspace-rejected-v1", "backpressure-v1", "plugin-update-v1", "host-runtime-diagnostics-v1"]`, and `operations` generated by sorting
...
The optional `host-runtime-diagnostics-v1` feature advertises the read-only
`host_runtime.status` operation. Its payload is exactly `{}`. Discover it through
`agents.list {"linkProtocol":1}` and consume negotiated capabilities even when
that request fails. No fields are added to `socket.ready` or legacy envelopes.
Missing advertised support means diagnostics are unavailable, not that Hermes is
outdated. Scope client snapshots to the authenticated host and transport generation.

The completed workspace result has this exact payload schema (all keys present):
...
hermes:
  ...
  updateState: "unknown"|"current"|"available"
  updateCheckedAt: integer|null (Unix seconds)
  restartState: "unknown"|"not_required"|"required"
plugin:
  runningVersion: string
  installedRevision: string|null (40 lowercase hexadecimal characters)
  activeRevision: string|null (40 lowercase hexadecimal characters)
  restartState: "unknown"|"not_required"|"required"
compatibility:
  state: "unknown"|"compatible"|"incompatible"
  checkedOperations: string[]
  unavailableOperations: string[]
  issues: [{code: string, operation: string, message: string, suggestedAction: string}]
```

`runtimeId` and `plugin.activeRevision` use the updater's module-load nonce and
revision, never an earlier process's journal. `installedRevision` uses the
existing plugin-owned installation identity reader for this module's root.
Two known, unequal revisions mean plugin restart is `required`; equal revisions
mean `not_required` for this plugin only. Missing identity means `unknown`.
An old `up_to_date` operation is neither current-release nor activation evidence.

`hermes.cliVersion` is only the version of `hermes --version` resolved by
the host's PATH, not proof of the connected gateway's loaded code or even the
same installation. The read has a three-second timeout (plus at most one second
for child cleanup), a 4 KiB output budget, no shell, and a five-minute
process-local cache (including unavailable results).
Only a bounded version token is exported; paths, raw output and exceptions are
never returned. No version probe runs during agent discovery. This implementation
has no supported source for the running gateway version or fresh update
availability: `runningVersion` and `updateCheckedAt` are `null`, and Hermes
`updateState` and `restartState` are `unknown`. Do not interpret CLI behindness,
missing metadata, disconnects, or the observed capability error as an available
update. The other enum values are reserved for positively verified evidence.

Compatibility covers only observed `agents.list`, not the entire advertised
operation catalog. Before observation, both operation arrays and `issues` are
empty and state is `unknown`. A successful enumeration sets `compatible`, with
`checkedOperations: ["agents.list"]` and no unavailable operations or issues.
The exact missing `hermes_cli.profiles.list_profile_names` import sets
`incompatible`, both operation arrays to `["agents.list"]`, and one issue:

```json
{"code":"hermes_capability_missing","operation":"agents.list","message":"This Hermes gateway cannot list agent profiles. Update Hermes, restart the gateway, then reconnect Loopdy.","suggestedAction":"update_hermes"}
```

The failed enumeration returns the same code/message in the existing failed
workspace envelope. An unrelated import/I/O/permission failure is not classified
as missing capability. The observation is held by the current backend only and
is cleared by a later successful enumeration; diagnostics never invokes agent
listing itself. `observedAt` timestamps the snapshot, not an upstream update check.
Suggested actions are guidance only: this operation never updates, installs,
restarts, creates an updater journal, or changes profile configuration.
...
Host-local grants may use `--account` instead of explicit `--device` entries. Account scope uses the current account-key-bound pairing authority and selected profile while admitting any Link-authenticated device on that account. Link revocation/admission and host-target checks are unchanged. The host-only `wiki list` output adds `accessScope` (`device` or `account`); an account grant has `deviceIds: []`. Wire root DTOs are unchanged. Existing records migrate as device-scoped without rotating generations; only explicit host policy changes rotate generation. New `wiki.connect` registrations remain device-scoped and cannot request a scope change. Account grants survive process restarts, but a changed host pairing/authorization epoch or account key requires reauthorization.
