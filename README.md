...
To share an exact Wiki across all authenticated devices on the host's paired Loopdy account, use an explicit host-local account grant:

```bash
hermes loopdy wiki grant personal-wiki --root '/absolute/path/to/wiki' \
  --label 'Personal Wiki' --profile-id default --account \
  --access read-only --source-kind files --yes
```

`--account` replaces the extra Wiki device allowlist, not Link authentication. New phone/tablet registrations on that account need no new Wiki approval. Another account cannot use the grant. Folder confinement, selected profile, read-only policy, Link device revocation, and pairing-authority checks remain enforced. This scope includes other authenticated devices paired to the same account, including its other hosts; use `--device` if a narrower policy is intended. No public access or wildcard identity is introduced.

Grants persist across gateway restarts. Re-pairing the host, rotating its authorization generation, or changing its account key requires host-local reauthorization. The binding is the existing opaque pairing authority, which includes a fingerprint of the account key; it is not a display name or caller-provided account ID. Existing grants and new in-app registrations remain device-scoped unless an administrator explicitly converts a grant with `--account`. `wiki list` reports `accessScope` and `deviceIds`; account grants have an empty device list. Changing scope rotates the grant generation, so refresh old Wiki handles. Upgrade/restart every process using the store before using account scope; older plugin code fails closed on its empty device list.
...
On hosted installations with a custom, non-credential-named Hermes home such as `/opt/data`, an administrator can explicitly grant the exact Wiki subfolder with `hermes loopdy wiki grant`. This does not apply to the standard `~/.hermes` home or other credential/control-named paths: their descendants remain ungrantable, and the Wiki must live outside them. Add Wiki then reuses that authorized grant without changing its permissions or generation. It still cannot register unapproved folders under Hermes home or bypass account/profile ownership or an applicable device allowlist, overlapping grants, or filesystem safety checks. Enter the canonical absolute folder path without a trailing slash; the request contract does not normalize paths.
