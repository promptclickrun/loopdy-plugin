...
Host-local `wiki grant --account` explicitly authorizes all Link-authenticated devices on the captured paired account for one exact root/profile. The existing pairing authority hashes the relay origin, host identity/authorization epoch, and account-key fingerprint. The service checks that authority on every grant lookup and before/after I/O; the transport checks current ownership and target host before dispatch and response. Cross-account frames cannot decrypt with the paired account key. Device revocation and sender admission remain Link responsibilities, not a Wiki wildcard. The grant also covers other hosts paired to the same account. A host re-pair or account-key rotation fails closed and requires explicit reauthorization.

Account scope skips only the extra Wiki device allowlist. Existing rows migrate with `access_scope='device'` and unchanged generations/permissions; no grant widens automatically. The host CLI requires mutually exclusive `--account` or explicit `--device` values. Account grants store no device IDs, and old code therefore denies them. Scope changes rotate the generation and invalidate old revisions/uploads. Upload/recovery operations remain bound to their initiating device even on writable account grants, so another device cannot take over a pending save. No remote payload may select account scope or supply an account identity.
