...
def _coordinator_section(record: dict) -> str:
    ...
    return (
        ...
        f"{roster}\n"
        "Delegate scoped work with `thread_spawn` while you have a live turn; the "
        "Loopdy thread view can also launch session-backed workers out-of-turn. "
        "Review progress with `thread_status` and gather finished work with "
        "`thread_collect`; YOU assemble the final answer — never ask a worker to assemble it.\n"
