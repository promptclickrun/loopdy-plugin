...
class LiveRuntimeTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_repeated_provider_request_joins_pending_work_and_reports_to_both_delegations(self):
        platform_registry.register(PlatformEntry(name="loopdy",label="Loopdy",adapter_factory=lambda _:None,check_fn=lambda:True))
        class Client(_LinkClient):
            async def send_payload(self,payload,**kwargs):
                self.payloads.append(payload)
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            try:
                await runtime.dispatch(None,{**base,"type":"voice.live.offer","sdp":"v=0\r\nm=audio 9 UDP/TLS/RTP/SAVPF 111\r\n"},route)
                provider=providers[0]
                for id in ("delegation_first", "delegation_second"):
                    await provider.callback({"kind":"delegation","id":id,"text":"First task"})
                async with asyncio.timeout(3):
                    while not seen: await asyncio.sleep(.01)
                await asyncio.sleep(.05)
                self.assertEqual(len(seen), 1, "Repeated provider delegation must not execute the same pending request twice")
                gates["First task"].set()
                async with asyncio.timeout(3):
                    while len(provider.results)<2: await asyncio.sleep(.01)
                self.assertEqual({row[0] for row in provider.results}, {"delegation_first", "delegation_second"})
                self.assertFalse(provider.closed)
                await provider.callback({"kind":"provider_error", "fatal":False, "code":"upstream_error"})
                await asyncio.sleep(.05)
                self.assertFalse(provider.closed, "A recoverable provider error must keep the call open")
                await provider.callback({"kind":"delegation", "id":"delegation_second", "text":"First task"})
                await asyncio.sleep(.03)
                self.assertEqual(len(seen), 1, "The same alias never replays completed work")
                with self.assertRaises(ValueError):
                    await provider.callback({"kind":"delegation", "id":"delegation_second", "text":"Changed task"})
                await provider.callback({"kind":"delegation", "id":"delegation_later", "text":"First task"})
                async with asyncio.timeout(3):
                    while len(seen) < 2: await asyncio.sleep(.01)
                self.assertEqual(len(seen), 2, "A fresh request after completion is allowed")
            finally:
                for gate in gates.values(): gate.set()
                await runtime.shutdown()
